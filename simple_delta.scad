...
// TODO Daniel: Replace lm8uu with IGUS RJMP 01-08
l_lm8uu = 25 + 3; // 3 mm offset
od_lm8uu = 16;
...
carriage_offset = 19; // TODO: Daniel changed because of IGUS bearings which have 1mm larger diameter
...
d_ball_bearing = 9;//3 * 25.4 / 8;
//id_magnet = 15 * 25.4 / 64;
od_magnet = 10 - 0.2;//3 * 25.4 / 8; // TODO Daniel a bit smaller for tight fitting
h_magnet = 3 + 0.1;//25.4 / 8;
// TODO: Daniel - fix r_bearing_seated!!!
r_bearing_seated = 1;//pow(pow(d_ball_bearing / 2, 2) - pow(id_magnet / 2, 2), 0.5); // depth ball bearing sinks into magnet id
		...
		hull() {
			...
			translate([0, -l_mount + w_mount / 2, 0])
				cube([w_mount, w_mount, t_clamp], center = true);
						...
						// HACK Daniel
                       translate([0, -l_mount + w_mount / 2 + 21, 0])
                                      cube([w_mount, w_mount + 42, t_clamp], center = true);   
		}
...
d_metal_screw = 3.5; // TODO Daniel, we use 3.5mm metal sheet screws
				...
				hull()
					for (k = [-1, 1])
						translate([0, k * l_linking_board_slot / 2, 0])
							cylinder(r = d_metal_screw / 2, h = w_mount + 2, center = true);
			else
				cylinder(r = d_metal_screw / 2, h = w_mount + 2, center = true);
}
...
module mount(
	l_linking_board_slot) {
mount_d_screw_offset=0.5; // TODO Daniel: Larger holes for metal sheet screws
	union() {
		mount_body(
			...
			translate([-w_mount / 2 - w_base_mount / 2 + r_base_mount, -l_base_mount / 2, (t_base_mount - t_clamp) / 2])
				difference() {
					...
					translate([-w_base_mount / 2 + 4, -l_base_mount / 2 + 4, 0])
						cylinder(r = d_metal_screw / 2, h = t_base_mount + 1, center = true);
		...
		for (i = [-1, 1])
			translate([i * cc_guides / 2, 0, 0]) {
				translate([0, 0, z_offset_guides]) {
					...
					cube([gap_clamp, 0.4*w_clamp, thickness], center = true);
				}

				//translate([0, w_clamp / 2, 0])
				//	cube([gap_clamp, w_clamp/2, thickness + 2 - 2], center = true);
			}
	}
}
...
module carriage(
	...
	d_guides_offset = 2; // TODO: Daniel: offset for more space around rods
	...
	difference() {
		...
		for (i = [-1, 1])
			translate([i * cc_guides / 2, 0, 0]) {
				...
				hull() {
					cylinder(r = (d_guides + d_guides_offset) / 2, h = t_carriage + 1, center = true); // TODO Daniel Offset more space for rods
		...
		translate([0, 0, h_groove_jhead]) {
			...
			translate([0, 0, h_groove_offset_jhead + 0.25]) {
				...
				translate([0, 0, 175_bowden[1] + 0.25])
					cylinder(r = 175_bowden[2] / 2 + 0.5, h = 30);
